...
const toolboxGuides = {
  media: {
    ...
    tips: ['输出目录请选和输入文件夹不同的位置，避免原文件被覆盖', '处理大文件时不要关闭程序，中断可重新开始'],
  },
  asr: {
    ...
    tips: ['索引存在程序内部（界面有显示路径）；素材文件移动位置后需重新索引', 'large-v3 模型较大（约5.4GB），首次使用需单独下载模型包', '素材库新增视频后再点一次「开始索引」，只处理新增，已索引的自动跳过'],
  },
  subtitle: {
    ...
    tips: ['批量模式必须先到「语音识别与索引」建好字幕索引', '单文件模式请提前准备好 .srt 或 .ass 字幕文件，且文件名与视频一致'],
  },
  cut: {
    ...
    tips: ['建议先点「预览检测」看会剪掉哪些停顿，不满意再调参数', '原文件不会被改动，结果都输出到你指定的目录'],
  },
  tts: {
    ...
    tips: ['生成的 WAV 是纯音频，可在「生成与结果」页作为口播配音使用', '长文本会自动分段合成后拼接成一条', '已从索引导入过文本后，需清空文本框才能重新导入'],
  },
  match: {
    ...
    tips: ['素材库必须先到「语音识别与索引」建好索引，否则无法匹配', '文案每行一句，逐句匹配效果最好', '完成后「匹配报告」会显示每句用了哪个素材片段、用了几次'],
  },
};
