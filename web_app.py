...
class Handler(BaseHTTPRequestHandler):
    ...
    def _make_config(self, payload: dict) -> JobConfig | str:
        ...
        return JobConfig(
            ...
            im_wm_enabled=bool(payload.get("im_wm_enabled", False)),
            im_wm_source=_safe_choice(str(payload.get("im_wm_source", "auto")), {"auto", "custom"}, "auto"),
            im_wm_image=str(payload.get("im_wm_image") or "").strip(),
            im_wm_motion=_safe_choice(str(payload.get("im_wm_motion", "static")), {"static", "random"}, "static"),
            im_wm_density=_safe_choice(str(payload.get("im_wm_density", "frame")), {"frame", "2s"}, "frame"),
            im_wm_opacity=max(0.0001, min(1.0, float(payload.get("im_wm_opacity", 0.02) or 0.02))),
            filter_enabled=bool(payload.get("filter_enabled", False)),
            filter_mode=_safe_choice(str(payload.get("filter_mode", "random")), {"random", "custom"}, "random"),
            filter_name=str(payload.get("filter_name", "warm")),
            filter_lut_path=str(payload.get("filter_lut_path") or "").strip(),
            filter_strength=max(0.1, min(100.0, float(payload.get("filter_strength", 20) or 20))),
